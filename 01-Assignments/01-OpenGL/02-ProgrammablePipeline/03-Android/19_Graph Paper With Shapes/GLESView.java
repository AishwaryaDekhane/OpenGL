...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private int [] vbo_rectangle_pos = new int [1];
	private int [] vbo_outer_circle_pos = new int [1];
	...
	private int [] vbo_rectangle_col = new int [1];
	private int [] vbo_outer_circle_col = new int [1];
...
//--------------------------------------------------------------------------------------------------------------------------------------------
	...
	private void initializeReactangle()
	{
		final float [] reactanglepos = new float []
		{
			-0.5f, -0.5f, 0.0f,
			0.5f, -0.5f, 0.0f,
			-0.5f, -0.5f, 0.0f,
		...
		final float [] rectangleCol = new float []
		{
			1.0f, 1.0f, 0.0f,
			1.0f, 1.0f, 0.0f,
			1.0f, 1.0f, 0.0f,
		...
		//create vbo
		GLES31.glGenBuffers(1, vbo_rectangle_pos, 0);

		//target vbo <-> GL_ARRAY_BUFFER
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_rectangle_pos[0]);
		...
		ByteBuffer byteBufferRecPos = ByteBuffer.allocateDirect(reactanglepos.length * 4);	
		...
		positionBufferRec.put(reactanglepos);
		...
		//fill the buffer data statically
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, reactanglepos.length * 4, positionBufferRec, GLES31.GL_STATIC_DRAW);
		...
		//create vbo
		GLES31.glGenBuffers(1, vbo_rectangle_col, 0);

		//target vbo <-> GL_ARRAY_BUFFER
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_rectangle_col[0]);
		...
		ByteBuffer byteBufferRecColor = ByteBuffer.allocateDirect(rectangleCol.length * 4);	
		...
		colorBufferRec.put(rectangleCol);
		...
		//fill the buffer data statically
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, rectangleCol.length * 4, colorBufferRec, GLES31.GL_STATIC_DRAW);
	...
	private void initializeOuterCircle()
	{
		float [] outerCircleVertex = new float [10];
		float radius_outer = 0.71f;
		...
		*/
		...
		final float [] outerCircleCol = new float []
		{
			1.0f,1.0f,0.0f,
			1.0f,1.0f,0.0f,
		...
		GLES31.glGenVertexArrays(1, vao_outer_circle, 0);
		GLES31.glBindVertexArray(vao_outer_circle[0]);
	
		for (float angle = 0.0f; angle < (2.0f * PI); angle = angle + 0.01f)
		{
			outerCircleVertex[0] = (float)((Math.cos(angle)) * radius_outer);
			outerCircleVertex[1] = (float)((Math.sin(angle)) * radius_outer);
			outerCircleVertex[2] = 0.0f;
		}
	
		GLES31.glGenBuffers(1, vbo_outer_circle_pos, 0);
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_outer_circle_pos[0]);
		ByteBuffer byteBufferOCPos = ByteBuffer.allocateDirect(outerCircleVertex.length * 4);
		byteBufferOCPos.order(ByteOrder.nativeOrder());
		FloatBuffer positionBufferOC = byteBufferOCPos.asFloatBuffer();
		positionBufferOC.put(outerCircleVertex);
		positionBufferOC.position(0);
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, outerCircleVertex.length * 4, positionBufferOC, GLES31.GL_DYNAMIC_DRAW);
		GLES31.glVertexAttribPointer(GLESMacros.AMC_ATTRIBUTE_POSITION, 3, GLES31.GL_FLOAT, false, 0, 0);
		GLES31.glEnableVertexAttribArray(GLESMacros.AMC_ATTRIBUTE_POSITION);	//AMC = AstroMediComp
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, 0);
	
		GLES31.glGenBuffers(1, vbo_outer_circle_col, 0);
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, vbo_outer_circle_col[0]);
		ByteBuffer byteBufferOCColor = ByteBuffer.allocateDirect(outerCircleCol.length * 4);	
		byteBufferOCColor.order(ByteOrder.nativeOrder());
		FloatBuffer colorBufferOC = byteBufferOCColor.asFloatBuffer();
		colorBufferOC.put(outerCircleCol);
		colorBufferOC.position(0);
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER, outerCircleCol.length * 4, colorBufferOC, GLES31.GL_STATIC_DRAW);
		GLES31.glVertexAttribPointer(GLESMacros.AMC_ATTRIBUTE_COLOR, 3, GLES31.GL_FLOAT, false, 0, 0);  //false = we are not sending normalized co-or
		GLES31.glEnableVertexAttribArray(GLESMacros.AMC_ATTRIBUTE_COLOR);	//AMC = AstroMediComp
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER, 0);
		GLES31.glBindVertexArray(0);
	...
	private void display()
	{
		...
		//Translates matrix m by x, y, and z in place.
		//translationMatrix = translate(-1.5f, 0.0f, -6.0f);
		Matrix.translateM(modelViewMatrix, 0, 0.003f, 0.003f, -2.0f);
	...
	void uninitialize()
	{
		...
		if (vbo_rectangle_pos[0] != 0)
		{
			GLES31.glDeleteVertexArrays(1, vbo_rectangle_pos,0);
			vbo_rectangle_pos[0] = 0;
		}

		if (vbo_rectangle_col[0] != 0)
		{
			GLES31.glDeleteVertexArrays(1, vbo_rectangle_col,0);
			vbo_rectangle_col[0] = 0;
		}
		...
		if (vao_outer_circle[0] != 0)
		{
			GLES31.glDeleteVertexArrays(1, vao_outer_circle,0);
			vao_outer_circle[0] = 0;
		}

		if (vbo_outer_circle_pos[0] != 0)
		{
			GLES31.glDeleteVertexArrays(1, vbo_outer_circle_pos,0);
			vbo_outer_circle_pos[0] = 0;
		}

		if (vbo_outer_circle_col[0] != 0)
		{
			GLES31.glDeleteVertexArrays(1, vbo_outer_circle_col,0);
			vbo_outer_circle_col[0] = 0;
		}
