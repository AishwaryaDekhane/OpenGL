...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Pyramid Animating Textured"),
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescPyramidTexture, NULL,
	...
	//PYRAMID
	//create texture
	hr = LoadD3DTexture(L"TRU.bmp", &gpID3D11ShaderResourceView_Pyramid_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	hr = gpID3D11Device->CreateSamplerState(&samplerDesc_pyramid, &gpID3D11SamplerState_Pyramid_Texture);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
