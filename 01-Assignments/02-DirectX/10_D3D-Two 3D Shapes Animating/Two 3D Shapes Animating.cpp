...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Two 3D Colored Shapes Animating"),
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDescPyramidPosition, 
		NULL,
	...
	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescPyramidColor;
	ZeroMemory(&bufferDescPyramidColor, sizeof(D3D11_BUFFER_DESC));
	bufferDescPyramidColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescPyramidColor.ByteWidth = sizeof(float) * ARRAYSIZE(pyramidColor);
	bufferDescPyramidColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescPyramidColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescPyramidColor, NULL,
		&gpID3D11Buffer_VertexBuffer_Pyramid_Color);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Failed.\n");
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourcePyramidColor;
	ZeroMemory(&mappedSubresourcePyramidColor, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Pyramid_Color,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourcePyramidColor);
	memcpy(mappedSubresourcePyramidColor.pData, pyramidColor, sizeof(pyramidColor));
	...
	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescCubeColor;
	ZeroMemory(&bufferDescCubeColor, sizeof(D3D11_BUFFER_DESC));
	bufferDescCubeColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescCubeColor.ByteWidth = sizeof(float) * ARRAYSIZE(cubeColor);
	bufferDescCubeColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescCubeColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescCubeColor, NULL,
		&gpID3D11Buffer_VertexBuffer_Cube_Color);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Failed.\n");
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceCubeColor;
	ZeroMemory(&mappedSubresourceCubeColor, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Cube_Color,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceCubeColor);
	memcpy(mappedSubresourceCubeColor.pData, cubeColor, sizeof(cubeColor));
...
void display(void)
{
	...
	XMMATRIX rotationMatrix = XMMatrixRotationY(-angle_rotate);
