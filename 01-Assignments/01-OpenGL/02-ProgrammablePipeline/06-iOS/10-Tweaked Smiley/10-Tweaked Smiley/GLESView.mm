...
- (id)initWithFrame:(CGRect)frameRect
{
    ...
    if(self)
    {
        ...
        tweaked_smiley = [self loadTexture:@"Smiley" :@"bmp"];
...
-(void)drawView:(id)sender
{
    ...
    GLfloat rec_texcoord[8];
    GLfloat rec_vertices[12];
    ...
    //bind with texture
    glBindTexture(GL_TEXTURE_2D, tweaked_smiley);
    ...
    if (PressedDigit == 1)
    {
        ...
        rec_vertices[8] = 0.0f;
        rec_vertices[9] = 1.0f;
        rec_vertices[10] = -1.0f;
        rec_vertices[11] = 0.0f;
    ...
    //vertices
    glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(rec_vertices), rec_vertices, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    
    //texture
    glBindBuffer(GL_ARRAY_BUFFER, vbo_texture);
    glBufferData(GL_ARRAY_BUFFER, sizeof(rec_texcoord), rec_texcoord, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
...
- (void)dealloc
{
    ...
    if(tweaked_smiley)
    {
        glDeleteTextures(1, &tweaked_smiley);
        tweaked_smiley = 0;
    }
