...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Geometry Shader"),
...
HRESULT initialize(void)
{
	...
	hr = D3DCompile(geometryShaderSourceCode,
		lstrlenA(geometryShaderSourceCode),
		"GS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "D3DCompile() is Failed For Geometry Shader :%s \n", (char *)pID3DBlob_Error->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "D3DCompile() is Succeeded For Geometry Shader.\n");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateGeometryShader() is Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateGeometryShader() is Succeeded.\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	UINT stride = sizeof(float) * 3;	//3 is x,y,z
	UINT offset = 0;
