...
void initialize(void)
{
	...
	glVertexAttribPointer(AMC_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL); // 3 is for r,g,b in cubeColors array
...
void display(void)
{
	...
	ratationMatrix = rotate(gAnglePyramid, 0.0f, 1.0f, 0.0f);
	...
	// 3 (each with its x,y,z ) vertices for 4 faces of pyramid
	...
	ratationMatrix = rotate(gAngleCube, gAngleCube, gAngleCube);
...
{
	// code
	gAnglePyramid = gAnglePyramid + 0.05f;
	if (gAnglePyramid >= 360.0f)
		gAnglePyramid = gAnglePyramid - 360.0f;

	gAngleCube = gAngleCube + 0.05f;
