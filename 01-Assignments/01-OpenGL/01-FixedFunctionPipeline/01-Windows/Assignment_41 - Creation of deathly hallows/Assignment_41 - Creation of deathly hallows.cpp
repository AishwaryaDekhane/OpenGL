...
void display(void)
{
	...
	static GLfloat x1 = -2.0f;
	static GLfloat y1 = -2.0f;
	...
	glTranslatef(x1, y1, -2.5f);       //translation should be same for both the geometry
	...
	static GLfloat x2 = 2.0f;
	static GLfloat y2 = -2.0f;

	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();

	glTranslatef(x2, y2, -2.5f);
...
/*for (GLfloat i = 0.05f;i >= 0.0f;i=i-0.1f)
//{
glTranslatef(0.0f, i, -2.5f);
glRotatef(angle_line, 0.0f, 0.0f, 0.0f);
