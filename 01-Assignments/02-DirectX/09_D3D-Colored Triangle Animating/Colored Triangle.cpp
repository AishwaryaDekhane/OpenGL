...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Colored Animating Triangle"),
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_position, NULL,
		&gpID3D11Buffer_VertexBuffer_Position);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Failed for vertices.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Succeeded for vertices.\n");
		fclose(gpFile);
	}
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_color, NULL,
		&gpID3D11Buffer_VertexBuffer_Color);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Failed for color.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "gpID3D11Device::CreateBuffer() is Succeeded for color.\n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	XMMATRIX rotationMatrix = XMMatrixRotationY(angle_tri);
