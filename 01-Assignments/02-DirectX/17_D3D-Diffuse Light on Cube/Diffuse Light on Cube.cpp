...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Diffuse Light on Cube"),
...
HRESULT initialise(void)
{
	...
	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescCubeNormal;
	ZeroMemory(&bufferDescCubeNormal, sizeof(D3D11_BUFFER_DESC));
	bufferDescCubeNormal.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescCubeNormal.ByteWidth = sizeof(float) * ARRAYSIZE(cubeNormals);
	bufferDescCubeNormal.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescCubeNormal.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescCubeNormal, NULL,
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceCubeNormal;
	ZeroMemory(&mappedSubresourceCubeNormal, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Cube_Normal,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceCubeNormal);
	memcpy(mappedSubresourceCubeNormal.pData, cubeNormals, sizeof(cubeNormals));
...
void update(void)
{
	rotate_angle = rotate_angle + 0.0005f;
...
void display(void)
{
	...
	//All Axis Rotation
	XMMATRIX rotation_X = XMMatrixRotationX(rotate_angle);
	XMMATRIX rotation_Y = XMMatrixRotationY(rotate_angle);
	XMMATRIX rotation_Z = XMMatrixRotationZ(rotate_angle);
...
void uninitialise(void)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "uninitialise() is Succeeded.\n");
