...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Two 2D Shapes Black and White"),
...
HRESULT initialize(void)
{
	...
	ZeroMemory(&bufferDescTriangle, sizeof(D3D11_BUFFER_DESC));
	bufferDescTriangle.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescTriangle.ByteWidth = sizeof(float) * ARRAYSIZE(triangleVertices);
	bufferDescTriangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescTriangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescTriangle, NULL,
		&gpID3D11Buffer_VertexBuffer_Triangle);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceTriangle;
	ZeroMemory(&mappedSubresourceTriangle, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceTriangle);
	memcpy(mappedSubresourceTriangle.pData, triangleVertices, sizeof(triangleVertices));
	...
	bufferDescRectangle.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleVertices);
	bufferDescRectangle.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescRectangle.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescRectangle, NULL,
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceRectangle;
	ZeroMemory(&mappedSubresourceRectangle, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceRectangle);
	memcpy(mappedSubresourceRectangle.pData, rectangleVertices, sizeof(rectangleVertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Rectangle, NULL);
