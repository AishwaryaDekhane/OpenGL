	...
	while (bDone == false)
	{
		while (XPending(gpDisplay))
		{
			...
			case KeyPress: //parallel to WM_KEYDOWN
				...
				case XK_S: Sholder = (Sholder + 3) % 360;
					break;

				case XK_s: Sholder = (Sholder - 3) % 360;
					break;

				case XK_E: Elbow = (Elbow + 3) % 360;
					break;
...
void display(void)
{
	...
	ratationMatrix = rotate((GLfloat)Sholder, 0.0f, 0.0f, 1.0f);
	...
	ratationMatrix = rotate(90.0f, 1.0f, 0.0f, 0.0f);
	modelViewMatrix = modelViewMatrix * ratationMatrix;

	ratationMatrix = rotate((GLfloat)Elbow, 0.0f, 0.0f, 1.0f);
	modelViewMatrix = modelViewMatrix * ratationMatrix;
