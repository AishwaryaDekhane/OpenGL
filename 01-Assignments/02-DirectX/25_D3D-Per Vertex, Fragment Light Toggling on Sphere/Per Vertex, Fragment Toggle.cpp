...
ID3D11Buffer* gpID3D11Buffer_ConstantBuffer_perVertex = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Per Vertex, Fragment Light Toggling on Sphere"),
...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode_perVertex =
		...
		"{" \
			...
			"float3 transformed_normal = normalize(mul((float3x3)worldMatrix, (float3)normal));" \
			"float3 light_direction = (float3)normalize(lightPosition - eye_coordinates);" \
			"float tn_dot_ld = max(dot(transformed_normal, light_direction), 0.0);" \
	...
	hr = D3DCompile(vertexShaderSourceCode_perVertex,
		lstrlenA(vertexShaderSourceCode_perVertex),
		"VS",
		NULL,
		...
		0,
		0,
		&pID3DBlob_VertexShaderSourceCode_perVertex,
		&pID3DBlob_Error);
	...
	hr = gpID3D11Device->CreateVertexShader(pID3DBlob_VertexShaderSourceCode_perVertex->GetBufferPointer(),		//shader's binary code
		pID3DBlob_VertexShaderSourceCode_perVertex->GetBufferSize(),													//size of that code
		NULL,																								//class linkage parameter
		&gpID3D11VertexShader_perVertex);																				//Empty Utensil To Give The Vertex Shader Back
	...
	ID3DBlob* pID3DBlob_PixelShader_perVertex = NULL;
	pID3DBlob_Error = NULL;

	hr = D3DCompile(pixelShaderSourceCode_perVertex,
		lstrlenA(pixelShaderSourceCode_perVertex),
		"PS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
		...
		0,
		0,
		&pID3DBlob_PixelShader_perVertex,
		&pID3DBlob_Error);
	...
	hr = gpID3D11Device->CreatePixelShader(pID3DBlob_PixelShader_perVertex->GetBufferPointer(),		
		pID3DBlob_PixelShader_perVertex->GetBufferSize(),													
		NULL,																					
	...
	hr = gpID3D11Device->CreateInputLayout(inputElementDesc,
		...
		pID3DBlob_VertexShaderSourceCode_perVertex->GetBufferPointer(),
		pID3DBlob_VertexShaderSourceCode_perVertex->GetBufferSize(),
	...
	pID3DBlob_PixelShader_perVertex->Release();
	pID3DBlob_PixelShader_perVertex = NULL;
	...
	D3D11_BUFFER_DESC bufferDesc_ConstantBuffer_perVertex;
	ZeroMemory(&bufferDesc_ConstantBuffer_perVertex, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBuffer_perVertex.Usage = D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBuffer_perVertex.ByteWidth = sizeof(CBUFFER);
	bufferDesc_ConstantBuffer_perVertex.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_perVertex, nullptr,
		&gpID3D11Buffer_ConstantBuffer_perVertex);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	//define constant buffer
	D3D11_BUFFER_DESC bufferDesc_ConstantBuffer_perFragment;
	ZeroMemory(&bufferDesc_ConstantBuffer_perFragment, sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBuffer_perFragment.Usage = D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBuffer_perFragment.ByteWidth = sizeof(CBUFFER);
	bufferDesc_ConstantBuffer_perFragment.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_perFragment, nullptr,
...
void display(void)
{
	...
	CBUFFER constantBuffer_perVertex, constantBuffer_perFragment;
	constantBuffer_perVertex.WorldMatrix = worldMatrix;
	constantBuffer_perVertex.ViewMatrix = viewMatrix;
	constantBuffer_perVertex.ProjectionMatrix = gPerspectiveProjectionMatrix;

	constantBuffer_perFragment.WorldMatrix = worldMatrix;
	constantBuffer_perFragment.ViewMatrix = viewMatrix;
	constantBuffer_perFragment.ProjectionMatrix = gPerspectiveProjectionMatrix;

	if(bPerVertex == true)
	{
		if (gbLight == TRUE)
		{
			constantBuffer_perVertex.LKeyIsPressed = 1;
			constantBuffer_perVertex.La = XMVectorSet(LightAmbient[0],LightAmbient[1],LightAmbient[2],LightAmbient[3]);
			constantBuffer_perVertex.Ld = XMVectorSet(LightDiffuse[0],LightDiffuse[1],LightDiffuse[2],LightDiffuse[3]);
			constantBuffer_perVertex.Ls = XMVectorSet(LightSpecular[0],LightSpecular[1],LightSpecular[2],LightSpecular[3]);
			constantBuffer_perVertex.LightPosition = XMVectorSet(LightPosition[0],LightPosition[1],LightPosition[2],LightPosition[3]);

			constantBuffer_perVertex.Ka = XMVectorSet(MaterialAmbient[0],MaterialAmbient[1],MaterialAmbient[2],MaterialAmbient[3]);
			constantBuffer_perVertex.Kd = XMVectorSet(MaterialDiffuse[0],MaterialDiffuse[1],MaterialDiffuse[2],MaterialDiffuse[3]);
			constantBuffer_perVertex.Ks = XMVectorSet(MaterialSpecular[0],MaterialSpecular[1],MaterialSpecular[2],MaterialSpecular[3]);
			constantBuffer_perVertex.Material_Shininess = MaterialShininess;
		}
		else
		{
			constantBuffer_perVertex.LKeyIsPressed = 0;
		}
		...
		gpID3D11DeviceContext->VSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_perVertex);
		gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_perVertex, 0, NULL, &constantBuffer_perVertex, 0, 0);
	}
	...
	if(bPerFragment == true)
	{
		if (gbLight == TRUE)
		{
			constantBuffer_perFragment.LKeyIsPressed = 1;
			constantBuffer_perFragment.La = XMVectorSet(LightAmbient[0],LightAmbient[1],LightAmbient[2],LightAmbient[3]);
			constantBuffer_perFragment.Ld = XMVectorSet(LightDiffuse[0],LightDiffuse[1],LightDiffuse[2],LightDiffuse[3]);
			constantBuffer_perFragment.Ls = XMVectorSet(LightSpecular[0],LightSpecular[1],LightSpecular[2],LightSpecular[3]);
			constantBuffer_perFragment.LightPosition = XMVectorSet(LightPosition[0],LightPosition[1],LightPosition[2],LightPosition[3]);

			constantBuffer_perFragment.Ka = XMVectorSet(MaterialAmbient[0],MaterialAmbient[1],MaterialAmbient[2],MaterialAmbient[3]);
			constantBuffer_perFragment.Kd = XMVectorSet(MaterialDiffuse[0],MaterialDiffuse[1],MaterialDiffuse[2],MaterialDiffuse[3]);
			constantBuffer_perFragment.Ks = XMVectorSet(MaterialSpecular[0],MaterialSpecular[1],MaterialSpecular[2],MaterialSpecular[3]);
			constantBuffer_perFragment.Material_Shininess = MaterialShininess;
		}
		else
		{
			constantBuffer_perFragment.LKeyIsPressed = 0;
		}
		...
		gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer, 0, NULL, &constantBuffer_perFragment, 0, 0);
	}
...
{
	//code
	if (gpID3D11Buffer_ConstantBuffer_perVertex)
	{
		gpID3D11Buffer_ConstantBuffer_perVertex->Release();
		gpID3D11Buffer_ConstantBuffer_perVertex = NULL;
	}
