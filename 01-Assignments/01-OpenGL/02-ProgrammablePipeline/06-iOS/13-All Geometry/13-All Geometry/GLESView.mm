...
@implementation GLESView
{
    ...
    GLuint vbo_tri_pos;
    GLuint vbo_circle_pos;
    GLuint vbo_inner_circle_color;
    GLuint vbo_inner_circle_color_triangle;
    
    GLuint vbo_rec_pos;
    GLuint vbo_rec_pos_circle;
    GLuint vbo_rec_color;
    GLuint vbo_rec_color_circle;
...
- (id)initWithFrame:(CGRect)frameRect
{
    ...
    if(self)
    {
        ...
        glGenBuffers(1, &vbo_inner_circle_color_triangle);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_inner_circle_color_triangle);
        ...
        glGenBuffers(1, &vbo_circle_pos);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_circle_pos);
        glBufferData(GL_ARRAY_BUFFER, 1 * 3 * sizeof(GLfloat), NULL, GL_DYNAMIC_DRAW);
        glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        
        glGenBuffers(1, &vbo_inner_circle_color);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_inner_circle_color);
        glBufferData(GL_ARRAY_BUFFER, sizeof(circumscribedCircleColor), circumscribedCircleColor, GL_STATIC_DRAW);
        glVertexAttribPointer(AMC_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 0, NULL);
        glEnableVertexAttribArray(AMC_ATTRIBUTE_COLOR);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
        ...
        glGenBuffers(1, &vbo_rec_pos);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_rec_pos);
        ...
        glGenBuffers(1, &vbo_rec_color);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_rec_color);
        ...
        //circle
        glGenVertexArrays(1, &vao_rectangle_circle);
        glBindVertexArray(vao_rectangle_circle);
        
        glGenBuffers(1, &vbo_rec_pos_circle);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_rec_pos_circle);
        ...
        glGenBuffers(1, &vbo_rec_color_circle);
        glBindBuffer(GL_ARRAY_BUFFER, vbo_rec_color_circle);
...
{
    //code
    if (vbo_inner_circle_color_triangle)
    {
        glDeleteBuffers(1, &vbo_inner_circle_color_triangle);
        vbo_inner_circle_color_triangle = 0;
    }
    if (vbo_inner_circle_color)
    {
        glDeleteBuffers(1, &vbo_inner_circle_color);
        vbo_inner_circle_color = 0;
    }
    if (vbo_tri_pos)
    {
        glDeleteBuffers(1, &vbo_tri_pos);
        vbo_tri_pos = 0;
    }
    if (vbo_circle_pos)
    {
        glDeleteBuffers(1, &vbo_circle_pos);
        vbo_circle_pos = 0;
    }
    if (vbo_rec_color)
    {
        glDeleteBuffers(1, &vbo_rec_color);
        vbo_rec_color = 0;
    }
    if (vbo_rec_color_circle)
    {
        glDeleteBuffers(1, &vbo_rec_color_circle);
        vbo_rec_color_circle = 0;
    }
    if (vbo_rec_pos)
    {
        glDeleteBuffers(1, &vbo_rec_pos);
        vbo_rec_pos = 0;
    }
    if (vbo_rec_pos_circle)
    {
        glDeleteBuffers(1, &vbo_rec_pos_circle);
        vbo_rec_pos_circle = 0;
    }
    ...
    if (vao_rectangle_circle)
    {
        glDeleteVertexArrays(1, &vao_rectangle_circle);
        vao_rectangle_circle = 0;
    }
    //safe release
