...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szAppName,
		TEXT("Direct3D11 Two 2D Colored Shapes Animating Culling off"),
...
HRESULT initialize(void)
{
	...
	D3D11_BUFFER_DESC bufferDescTrianglePosition;
	ZeroMemory(&bufferDescTrianglePosition, sizeof(D3D11_BUFFER_DESC));
	bufferDescTrianglePosition.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescTrianglePosition.ByteWidth = sizeof(float) * ARRAYSIZE(triangleVertices);
	bufferDescTrianglePosition.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescTrianglePosition.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescTrianglePosition, NULL,
	...
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceTrianglePosition;
	ZeroMemory(&mappedSubresourceTrianglePosition, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle_Position,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceTrianglePosition);
	memcpy(mappedSubresourceTrianglePosition.pData, triangleVertices, sizeof(triangleVertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Triangle_Position, NULL);

	//color

	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescTriangleColor;
	ZeroMemory(&bufferDescTriangleColor, sizeof(D3D11_BUFFER_DESC));
	bufferDescTriangleColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescTriangleColor.ByteWidth = sizeof(float) * ARRAYSIZE(triangleColor);
	bufferDescTriangleColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescTriangleColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescTriangleColor, NULL,
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceTriangleColor;
	ZeroMemory(&mappedSubresourceTriangleColor, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Triangle_Color,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceTriangleColor);
	memcpy(mappedSubresourceTriangleColor.pData, triangleColor, sizeof(triangleColor));
	...
	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescRectanglePosition;
	ZeroMemory(&bufferDescRectanglePosition, sizeof(D3D11_BUFFER_DESC));
	bufferDescRectanglePosition.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescRectanglePosition.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleVertices);
	bufferDescRectanglePosition.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescRectanglePosition.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescRectanglePosition, NULL,
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceRectanglePosition;
	ZeroMemory(&mappedSubresourceRectanglePosition, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle_Position,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceRectanglePosition);
	memcpy(mappedSubresourceRectanglePosition.pData, rectangleVertices, sizeof(rectangleVertices));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Rectangle_Position, NULL);

	//color

	//create vertex buffer
	D3D11_BUFFER_DESC bufferDescRectangleColor;
	ZeroMemory(&bufferDescRectangleColor, sizeof(D3D11_BUFFER_DESC));
	bufferDescRectangleColor.Usage = D3D11_USAGE_DYNAMIC;
	bufferDescRectangleColor.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleVertices);
	bufferDescRectangleColor.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDescRectangleColor.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDescRectangleColor, NULL,
	...
	//copy vertices into above buffer
	D3D11_MAPPED_SUBRESOURCE mappedSubresourceRectangleColor;
	ZeroMemory(&mappedSubresourceRectangleColor, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Rectangle_Color,
		NULL,
		D3D11_MAP_WRITE_DISCARD,
		NULL,
		&mappedSubresourceRectangleColor);
	memcpy(mappedSubresourceRectangleColor.pData, rectangleColor, sizeof(rectangleColor));
...
void display(void)
{
	...
	//translation corncerned with world matrix transformation
	XMMATRIX worldMatrix = XMMatrixIdentity();
	XMMATRIX viewMatrix = XMMatrixIdentity();

	//transformation
	XMMATRIX translationMatrix = XMMatrixTranslation(-2.0f, 0.0f, 8.0f);
	...
	//transformation
	translationMatrix = XMMatrixTranslation(2.0f, 0.0f, 8.0f);
	rotationMatrix = XMMatrixRotationX(-RotationAngle);
	worldMatrix = rotationMatrix * translationMatrix;
